...
// Função que converte os dados do armazenamento local em Array Funcional e as lança na página principal.
(obtemDadosLs = () => {
    const dadosLS = localStorage.getItem('tarefas');    // 1. Obtemos os (dados brutos) do Armazenamento Local
    const converteJSON = JSON.parse(dadosLS);           // 2. Convertemos os dados, formato JSON (Array de String Puro) para (Array JS).

    for (i = 0; i < converteJSON.length; i++){          // 3. Criamos um loop que irá executar a função criadora de tarefas que receberá como argumento os dados da lista corrente.
